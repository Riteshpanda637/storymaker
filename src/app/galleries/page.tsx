...
const galleries = [
  {
    ...
    story: [
      ...
      "Their wedding was a celebration of traditions passed down through generations — the warmth of family, the glow of evening lamps, the sound of laughter echoing through temple corridors. Every ritual carried meaning, every moment carried weight.",
      ...
      "What struck me most was the stillness between them. In the middle of all the noise, all the celebrations, they kept finding each other. A look across the room. A hand held just a second longer.",
  ...
  },
  {
    ...
    story: [
      ...
      "There is something about the way they move together — effortless, natural, as though they have been walking side by side for lifetimes. He leads, she follows. She laughs, he smiles. The rhythm between them is something you cannot direct — only capture.",
  ...
  },
  {
    ...
    story: [
      ...
      "There is an ease to them that the camera loves. The way they walk together through crowded streets, the way the world blurs around them while they stay perfectly in focus. Long exposures caught buses streaking past, but nothing could move them.",
      "He in his earthy browns, she in her elegant white — they brought a warmth to every stone wall and cobbled path they stood against. No grand sets, no elaborate props. Just two people, deeply in love, letting the city tell their story.",
    ...
    photos: [
      ...
      "/SUNIL%20X%20SWETHA/DSC_8742-1.jpg",
    ],
  },
];
...
function GallerySlider({ photos }: { photos: string[] }) {
  ...
  const next = () => setSlideIndex((prev) => Math.min(prev + 1, maxIndex));
  const prev = () => setSlideIndex((prev) => Math.max(prev - 1, 0));

  return (
    <div className="gallery-slider">
      <button className="gallery-slider-arrow gallery-slider-arrow-left" onClick={prev}>
        <span>&#8249;</span>
      </button>

      <div className="gallery-slider-track-wrapper">
        <div
      ...
      <button className="gallery-slider-arrow gallery-slider-arrow-right" onClick={next}>
        <span>&#8250;</span>
      </button>
    </div>
  );
    ...
    <main className="home-page">
      ...
      <div className="galleries-content">
        {galleries.map((gallery, gIndex) => (
          <section key={gIndex} className="gallery-section">
            <h2 className="gallery-title">{gallery.title}</h2>
            <p className="gallery-date">{gallery.date}</p>

            <div className="gallery-story">
              {gallery.story.map((paragraph, pIndex) => (
                <p key={pIndex} className={pIndex === 0 ? "gallery-story-lead" : "gallery-story-text"}>
                  {paragraph}
                </p>
              ))}
            </div>
            ...
            <div className="gallery-video">
              <AutoPlayVideo src={gallery.video} />
            </div>

            {/* Photo Slider */}
            <GallerySlider photos={gallery.photos} />

            {/* Footer actions */}
            <div className="gallery-actions">
              <div className="gallery-actions-left">
                <span className="gallery-action-link">Comment</span>
                <span className="gallery-action-link">Share</span>
              </div>
            </div>
          </section>
        ))}
